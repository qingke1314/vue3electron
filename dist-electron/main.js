...
(function(exports2) {
  ...
  function getNetSession() {
    return require$$1$5.session.fromPartition(exports2.NET_SESSION_NAME, {
      cache: false
    });
  }
...
main$2.autoUpdater.logger.transports.file.level = "debug";
function createWindow() {
  win = new require$$1$5.BrowserWindow({
    width: 1400,
    height: 800,
    icon: path__namespace.join(process.env.APP_ROOT, "build/icons/icon.png"),
    webPreferences: {
  ...
  const menu = require$$1$5.Menu.buildFromTemplate(template);
  require$$1$5.Menu.setApplicationMenu(menu);
...
require$$1$5.app.whenReady().then(() => {
  createWindow();
  const ret = require$$1$5.globalShortcut.register("Control+Shift+D", () => {
    if (win) {
...
require$$1$5.app.on("will-quit", () => {
  require$$1$5.globalShortcut.unregisterAll();
});
require$$1$5.ipcMain.on("check-for-updates", () => {
  checkForUpdates();
});
require$$1$5.ipcMain.on("start-download", () => {
